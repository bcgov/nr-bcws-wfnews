...
export class MapStatePersistenceService {
    ...
    getMapState(): Promise<MapState> {
        if ( !this.mapUserPrefsRead ) return Promise.reject( Error( 'not read yet' ) )
    ...
    putMapState( mapState: MapState ): Promise<void> {
        if ( this.savingInProgress ) return Promise.reject( Error( 'save in progress' ) )

        var now = (new Date).getTime()
        if ( this.lastWrite && ( now - this.lastWrite ) < MINIMUM_WRITE_DELAY ) return Promise.reject( Error( 'not enough time since last save' ) )

        var newStateHash = UtilHash( mapState )
        if ( this.lastMapStateHash == newStateHash ) return Promise.reject( Error( 'same hash as last save' ) )
    ...
    getNonMapPrefs(): Promise<any> {
        if ( !this.nonMapUserPrefsRead ) return Promise.reject( Error( 'not read yet' ) )
        return this.nonMapUserPrefsRead
    }

    private parseMapUserPrefs( result: any ): MapState {
        if ( !result[ 'preferences' ] ) throw Error( 'preferences missing' )

        let pref = result[ 'preferences' ].reduce( (acc, p) => {
        ...
        let mapPrefHeader = pref[mapStateKeyPrefix + '-header']
        if (mapPrefHeader) {
            try {
                let pieces = JSON.parse(mapPrefHeader.value)
                let mapPref = pieces.reduce((acc, p) => {
                    acc += pref[p].value
                    return acc
                }, '')
    ...
    private writeMapUserPrefs( mapState: MapState ): Promise<any> {
        if ( !this.userPrefsUrl ) return Promise.reject( Error( 'no user prefs url' ) )

        let state = JSON.stringify( mapState )
        let pieces = Math.trunc( state.length / MAX_PREF_SIZE ) + 1
        let statePieces = Array( pieces ).fill( 0 ).map( ( v, i ) => state.substr( i * MAX_PREF_SIZE, MAX_PREF_SIZE ) )

        let piecePrefs = statePieces.map( ( v, i ) => {
            return {
        ...
        return this.httpClient.post( this.userPrefsUrl, headerPref ).toPromise()
            .then( () => {
                return Promise.all( piecePrefs.map( ( p ) => {
                    return this.httpClient.post( this.userPrefsUrl, p ).toPromise()
                } ) )
            } )
    }
