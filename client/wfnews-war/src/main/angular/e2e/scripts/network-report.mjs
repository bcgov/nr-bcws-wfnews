...
function main() {
  ...
  out.push('');
  out.push('**The proxy covers the Payload only. It does not cover `CapacitorHttp`.**');
  out.push('This was measured on the device: with the device proxy set to a port where');
  out.push('nothing was listening, an Angular `HttpClient` read failed and a `CapacitorHttp`');
  out.push('read still returned 200. So `CapacitorHttp` ignores the Android system proxy.');
  out.push('');
  out.push('Three services send their requests that way, and their traffic is **not shaped');
  out.push('and not counted** in any table below:');
  out.push('');
  out.push('- `notification.service.ts` — the Saved Location reads');
  out.push('- `wf-map.service.ts` — the Active Wildfire Map');
  out.push('- `common-utility.service.ts`');
  out.push('');
  out.push('The `offline` Network Profile is the exception. It turns the Wi-Fi off, which');
  out.push('stops every path, so the offline rows are complete.');
  out.push('');
  ...
  out.push('|---|---|');
  out.push('| The order of the Network Profiles | **Yes**, for the Payload. Slower is slower. |');
  out.push('| The bytes for each Journey | **Too low.** The Payload only. `CapacitorHttp` is not counted. |');
