...
L.TimeDimension.Layer.ImageOverlay = L.TimeDimension.Layer.extend({
  ...
  eachLayer: function (method, context) {
    ...
    return L.TimeDimension.Layer.prototype.eachLayer.call(
      this,
      method,
      context,
    );
  ...
  _showLayer: function (layer, time) {
    ...
    if (this._timeCacheForward > -1) {
      ...
      if (objectsToRemove > 0) {
        remove = times.splice(
          index + this._timeCacheForward + 1,
          objectsToRemove,
        );
        this._removeLayers(remove);
      }
  ...
  _getLayerForTime: function (time) {
    ...
    newLayer.on(
      ...
      function (layer, time) {
        ...
        if (
          this._map.timeDimension &&
          time == this._map.timeDimension.getCurrentTime() &&
          !this._map.timeDimension.isLoading()
        ) {
          this._showLayer(layer, time);
        }
        this.fire("timeload", {
...
L.ImageOverlay.include({
  ...
  hide: function () {
    this._visible = false;
    if (this._image && this._image.style) this._image.style.display = "none";
  },

  show: function () {
    this._visible = true;
    if (this._image && this._image.style) this._image.style.display = "block";
  },
