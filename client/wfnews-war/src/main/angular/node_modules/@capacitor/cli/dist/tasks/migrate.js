...
async function patchOldCapacitorPlugins(config) {
    ...
    return await Promise.all(androidPlugins.map(async (p) => {
        ...
        if ((_b = (_a = p.manifest) === null || _a === void 0 ? void 0 : _a.android) === null || _b === void 0 ? void 0 : _b.src) {
            const buildGradlePath = (0, path_1.join)(config.app.rootDir, 'node_modules', p.id, p.manifest.android.src, 'build.gradle');
            const manifestPath = (0, path_1.join)(config.app.rootDir, 'node_modules', p.id, p.manifest.android.src, 'src', 'main', 'AndroidManifest.xml');
            ...
            if (!(gradleContent === null || gradleContent === void 0 ? void 0 : gradleContent.includes('namespace'))) {
                log_1.logger.warn(`${p.id} doesn't officially support Capacitor ${coreVersion} yet, doing our best moving it's package to build.gradle so it builds`);
