...
export class AdminContainer extends BaseContainer {
  collection$: Observable<any> = this.store.pipe(
    select(selectCurrentIncidentsSearch()),
  );
  searchState$: Observable<SearchState> = this.store.pipe(
    select(selectSearchState(SEARCH_INCIDENTS_COMPONENT_ID)),
  );
  loadState$: Observable<LoadState> = this.store.pipe(
    select(selectIncidentsLoadState()),
  );
  errorState$: Observable<ErrorState[]> = this.store.pipe(
