...
class DateRange<StartDate extends Date|Interval, EndDate extends Date|Present> {
    ...
    public static getOwlDate(date: Date): string {
         const formatter = Intl.DateTimeFormat('en-CA',{timeZone: 'America/Vancouver', hour12: false, year: 'numeric', month: 'numeric', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric', timeZoneName: undefined});
    ...
    public getCqlEpochSeconds(attributeName, present?: Date) {
        const epochSeconds = this.epochSeconds(present);
    ...
    public getCqlIsoDate(attributeName, present?: Date) {
        const absolute = this.fix(present);
...
return true;
        ...
        if(x instanceof Interval && x.equals(y)) {
return true;
}
        if(x instanceof Date && y instanceof Date && x.valueOf()==y.valueOf()) {
return true;
}
        return false;
    }
...
return false;
    ...
    public fix(present?: Date): DateRange<Date, Date> {
        ...
        const end: Date = this.end instanceof Date ? this.end : present;
    
        // Resolve an interval to a date that fare bebore the end
        const start = this.start instanceof Interval ? this.start.before(end) : this.start as Date;
    
        return new DateRange<Date, Date>(start, end);
    }
    
    public epochSeconds = function(present?: Date): any {
        const fixed = this.fix(present);
    ...
    public static parseWms(value: string|null|undefined): DateRange<any, any>|null|undefined {
        if (value === undefined ) {
            ...
            const [start, end] = value.split('/').map(part=>DateRange.parseWmsDate(part));
...
class Interval {
    ...
    public add(date: Date, multiplier: number = 1): Date {
        ...
        const oldValue = date['get'+this.unit.field]();
    ...
    public static parseWms(string: string) {
        const result = INTERVAL_WMS_REGEX.exec(string);
        const partials = INTERVAL_UNITS
            .map((unit, i)=>({unit, value:result[i+1] as any}))
            .filter(partial=>partial.value)
            .map(partial=>{
partial.value=parseInt(partial.value);return partial;
});
