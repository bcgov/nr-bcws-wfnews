    ...
    return [
        {
            ...
            popupTemplate: '<div class="smk-popup"><div class="popup-header"><mat-icon role="img" class="mat-icon notranslate material-icons mat-icon-no-color" aria-hidden="true" data-mat-icon-type="font">block</mat-icon><span class="title-label">Wildfire Bans and Prohibitions</span></div><div class="popup-title">{{feature.properties["TYPE"]}}</div><div class="popup-attributes"><div class="label">Date Active:</div><div class="attribute">{{new Date(feature.properties["ACCESS_STATUS_EFFECTIVE_DATE"]).toISOString().slice(0, 10)}}</div></div><div class="popup-attributes"><div class="label">Fire Centre:</div><div class="attribute">{{feature.properties["FIRE_CENTRE_NAME"]}}</div></div><div class="popup-attributes"><div class="label">Fire Bans:</div><div class="attribute attribute-red">{{feature.properties["ACCESS_PROHIBITION_DESCRIPTION"]}}</div></div><div class="popup-button-container"><a class="popup-button" target="_blank" rel="noopener" href="https://www2.gov.bc.ca/gov/content/safety/wildfire-status/fire-bans-and-restrictions">Learn More</a></div></div>',
            attributes: [
                {
