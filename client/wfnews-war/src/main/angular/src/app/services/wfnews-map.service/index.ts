...
export class WfimMapService {
    ...
    private timeoutLiveLayerRefresh() {
        ...
        window.setTimeout(()=> {self.timeoutLiveLayerRefresh()}, 30*1000)
    }
    ...
    installIdentifyHooks() {
        if ( !this.smkInstance.$tool.IdentifyListTool ) return

        // don't allow resizing identify area
        var displaySearchAreaOrig = this.smkInstance.$tool.IdentifyListTool.displaySearchArea
            ...
            'custom': ( ev ) => {
                ...
                if ( ev.layer.id.startsWith( 'bc-' ) ) {
                    ...
                    this.smkInstance.$viewer.updateLayersVisible().then( () => {
                        let ft = this.smkInstance.$viewer.identified.getPicked()
    ...
    installSmkHandlers() {
        ...
        this.wfMapService.setHandler( 'BespokeTool--zoom-to-box', 'activated', ( smk, tool ) => {
            let map = this.smkInstance.$viewer.map
    ...
    loadRofs( rofs: PublicReportOfFireResource[], clickCallback?: ClickCallback, temporary = false ): SmkPromise {
        return this.loadLayer( temporary ? 'temporary-rof' : 'rofs', clickCallback, () => {
            return rofs.map( rof => {
    ...
    loadNrofs( nrofs: ProvisionalZoneResource[], clickCallback: ClickCallback, temporary = false ): SmkPromise {
        return this.loadLayer( temporary ? 'temporary-nrof' : 'nrofs', clickCallback, () => {
            return nrofs.map( provZone => {
                ...
                // }
                var status = provZone.dismissedInd ? "Dismissed" : provZone.expiryTimestamp<new Date() ? "Expired" : "Active"
                return {
                    type: 'Feature',
                    properties: {
    ...
    isLayerVisible( layerId: string ): boolean {
        if ( !this.smkInstance ) return false
    ...
    zoomToTemporaryLayer( id: string ): SmkPromise {
        return this.smkInstancePromise.then( ( smk ) => {
            let lyg = smk.$viewer.acetate[ id ]
            let gj = lyg.toGeoJSON()
    ...
    getSelectedPoint(): Promise<LonLat> {
        return this.smkInstancePromise.then( ( smk ) => {
            if ( !this.pointMarkupFeature ) throw new Error( 'no selected point' )
    ...
    getSelectedPolygon(): Promise<any> {
        return this.smkInstancePromise.then( ( smk ) => {
            if ( !this.polygonMarkupFeature ) throw new Error( 'no selected polygon' )
    ...
    activateTool( toolId: string, activate = true ): SmkPromise {
        return this.smkInstancePromise.then( ( smk ) => {
            return smk.getToolById( toolId ).active = activate
        } )
    }
    ...
    setMapStateSaveHandler( handler: ( state: any ) => void ) {
        setInterval( () => {
            ...
            let cfg = this.smkInstance.getConfig()
            delete cfg.layers
            delete cfg.tools
            handler( cfg )
        }, 2000 )
    ...
    redrawMap(): SmkPromise {
        ...
        return this.refreshLayers( ( ly ) => {
            return this.isLayerVisible( ly.id )
        } )
    }
    // _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
    //
    ...
    zoomToAnchor() {
        ...
        return Promise.resolve()
            .then( () => {
                ...
                if ( !pt ) throw new Error( 'no anchor' )
            } )
    ...
    showCandidate( location?: Location ) {
        ...
        let anchor = this.placeData.getAnchor();
        return this.crosshairs2( location.loc )
            .then( () => {
                if ( anchor ) return this.arrow( anchor, location )
            } )
    }
    ...
    private crosshairs( pt?: LonLat ): SmkPromise {
        ...
        return this.putTemporaryLayer( 'crosshairs', () => {
            return window[ 'L' ].Layer.crosshairs( toLatLon( pt ) )
        } )
    }
    ...
    private crosshairs2( pt?: LonLat ): SmkPromise {
        ...
        return this.putTemporaryLayer( 'crosshairs2', () => {
            return window[ 'L' ].Layer.crosshairs( toLatLon( pt ), {
                style: {
                    opacity: 0.6,
                    fillOpacity: 0,
    ...
    private arrow( start?: LonLat, end?: Location ): SmkPromise {
        ...
        return this.putTemporaryLayer( 'arrow', () => {
            return window[ 'L' ].Layer.arrow( toLatLon( start ), toLatLon( end.loc ), {
    ...
    private refreshLayer( layerId: string ): SmkPromise {
        ...
        return this.smkInstancePromise
            .then( ( smk ) => {
                if ( !this.isLayerVisible( layerId ) ) return smk
	...
	private onTimeChange(time: DateRange<any, any>, affectedLayers: string[]){
		return this.refreshLayers(layer=> affectedLayers.includes(layer.id))
			.then(()=>{this.timeChange.emit(time)})
	}
	...
	getLayers(): Promise<SmkLayer[]> {
		return this.smkInstancePromise.then( ( smk ) => {
			return smk.$viewer.layerIds.map(id => smk.$viewer.layerId[id])
        })
	}
...
function leaflet() { return window[ 'L' ] }

function stripePattern(id, stripes, transform) {
    var width = stripes.map(stripe=>stripe[0]).reduce((a,b)=>a+b, 0)
