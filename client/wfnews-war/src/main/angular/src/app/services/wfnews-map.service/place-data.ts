...
export class PlaceData {
    ...
    updateResults(): Promise<any> {      
        ...
        return Promise.resolve()
            .then( function () { return searchPlaces( self ) } )
            .then( function () { return searchRoads( self ) } )
            .then( function () { return searchOccupants( self ) } )
            .then( function () { return searchAddresses( self ) } )
            .then( function () { return searchIntersections( self ) } )
            .then( 
                function () {
...
let fetchCache: { [url: string]: Promise<any> } = {}
...
function searchData( source: { n: string, t?: string, x: number, y: number }[], text: string, anchorPt: LonLat, maxDist?: number ): Location[] {
    ...
    for (var i = 0; i < source.length; i++) {
        ...
        if ( place.n.toLowerCase().startsWith( t ) ) {
            ...
            if ( maxDist && location.dist > maxDist ) continue
...
function removeDuplicateIntersections( locations ) {
    ...
    for (var i = 0; i < locations.length; i++) {
        ...
        if (i > 0) {
            var current = uniq[uniq.length-1];
            // skip if duplicate
...
function isDuplicateIntersection(intLoc1, intLoc2) {
    if (intLoc1.name !== intLoc2.name) return false;
    if (intLoc1.dist && intLoc2.dist) {
        var distanceBetween = intLoc1.dist - intLoc2.dist;
        if (distanceBetween > 1) return false;
    }
    return true;
}
...
function setAnchorData( anchorPt: LonLat, location: Location ) {
    ...
    if ( location.dist < EPSILON ) location.isAnchor = true
}
        ...
        .then( function ( result ) {
            var resultLoc = result.features
                .map( function ( feature ) {
                    ...
                    if ( ! feature.properties.intersectionName ) return;
