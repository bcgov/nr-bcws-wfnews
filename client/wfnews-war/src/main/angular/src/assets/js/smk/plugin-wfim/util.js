include.module( 'plugin-wfim-util', [], function () {
    ...
    function infoHtml( properties, attributeDef, options) {
        ...
        var clz = [options.className, "wfml-info-content"].filter(x=>x).join(' ');
    ...
    function makeEl ( tag, cls, opt ) {
        ...
        if ( opt.text ) 
            el.append( document.createTextNode( opt.text ) )

        if ( opt.title ) 
            el.setAttribute( 'title', opt.title )
    ...
    var TIME_FORMAT = Intl.DateTimeFormat('en-CA',{timeZone: undefined, hour12: false, year: 'numeric', month: 'numeric', day: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric', timeZoneName: 'short'})
    var DATE_FORMAT = Intl.DateTimeFormat('en-CA',{timeZone: undefined, hour12: false, year: 'numeric', month: 'numeric', day: 'numeric'})
    var FORMATTERS = {
    ...
    function infoPropDisplayValue(fprops, propFormat, parameter ) {
        ...
        return propFormat.replace(/\$\{\s*(.*?)\s*(?:\|\s*(.*?)\s*(?:\|\s*(.*?)\s*)?)?\}/gi, (match, names, formatter, defaultString) => {
            if(!defaultString) {
                defaultString=""
            }
            names = names.split(";")
