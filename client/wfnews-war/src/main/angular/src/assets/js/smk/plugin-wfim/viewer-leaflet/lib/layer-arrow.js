...
L.Layer.Arrow = L.LayerGroup.extend({
  ...
  var pt1 = this._map.project(this.locEnd);
	...
	var baseL = [ offset * cos + tailWidth * sin, offset * sin + tailWidth * -cos ];
	var baseR = [ offset * cos + tailWidth * -sin, offset * sin + tailWidth * cos ];
	var headL0 = [ tailLen * cos + headLen0 * sin, tailLen * sin + headLen0 * -cos ];
	var headL1 = [ tailLen * cos + headLen1 * sin, tailLen * sin + headLen1 * -cos ];
	var tipPt = [ len * cos, len * sin ];
	var headR0 = [ tailLen * cos + headLen0 * -sin, tailLen * sin + headLen0 * cos ];
	var headR1 = [ tailLen * cos + headLen1 * -sin, tailLen * sin + headLen1 * cos ];
