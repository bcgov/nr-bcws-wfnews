import {DateRange} from '../../wfml/date.js'
...
export default function (mapapi) {
    ...
    var layerIds = []
    function parseLayerIds() {
        layerIds = $( '#refresh-layer-ids' ).val()
            .split( /\s+/ )
            .map( function ( i ) { return i.trim() } )
            .filter( function ( i ) { return mapapi.layer.isValidId( i ) } )
    ...
    parseLayerIds()
    $( '#refresh-layer-ids' ).keyup( parseLayerIds ).change( parseLayerIds )
    ...
    parseTime()
    $( '#time-defn' ).keyup( parseTime ).change( parseTime )
    ...
    $( '#btn-map-redraw-layers' ).click( redrawLayers )
