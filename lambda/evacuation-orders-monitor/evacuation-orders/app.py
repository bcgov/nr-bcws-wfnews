...
def lambda_handler(event, context):
    ...
    monitor_name = "Evacuation Orders"
    # mm/dd/YY H:M:S
    ...
    if last_fetched_time_stamp is None:
        ...
        try:
            ...
            ip = requests.get( agol["layer_url"] + "/query?where=DATE_MODIFIED>'" +
                              ...
                              "=esriSRUnit_Meter&returnGeodetic=false&outFields=*&returnGeometry=true&returnCentroid"
                              "=false&featureEncoding=esriDefault&multipatchOption=xyFootprint&maxAllowableOffset"
                              "=&geometryPrecision=&outSR=4326&defaultSR=&datumTransformation=&applyVCSProjection"
                              "=false&returnIdsOnly=false&returnUniqueIdsOnly=false&returnCountOnly=false"
                              "&returnExtentOnly=false&returnQueryGeometry=false&returnDistinctValues=false&cacheHint"
                              "=false&orderByFields=&groupByFieldsForStatistics=&outStatistics=&having=&resultOffset"
                              "=&resultRecordCount=&returnZ=false&returnM=false&returnExceededLimitFeatures=true"
                              "&quantizationParameters=&sqlFormat=standard&f=pjson&token=", timeout=15)
        ...
        if ip is not None:
            ...
            for message in message_body["features"]:
                feature_date = datetime(1970, 1, 1) + timedelta(milliseconds=message['attributes']['DATE_MODIFIED'])
