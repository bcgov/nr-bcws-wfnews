...
export class SmkApi {
    ...
    panToFeature( geometry: GeoJsonTypes, zoomIn?, isMobile? ) {
        if ( !this.smkMap ) return

        this.smkMap.$viewer.panToFeature( geometry, zoomIn, isMobile )
    }
    ...
    getDiaplayContextItemTitle( itemId: string ) {
        return this.smkMap.$viewer.displayContext.layers.getItem( itemId ).title
    }
