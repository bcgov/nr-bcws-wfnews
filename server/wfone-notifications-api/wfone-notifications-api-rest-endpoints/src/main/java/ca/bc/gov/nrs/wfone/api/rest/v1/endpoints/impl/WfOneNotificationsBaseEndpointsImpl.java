...
public abstract class WfOneNotificationsBaseEndpointsImpl {

	private static final Logger logger = LoggerFactory.getLogger(WfOneNotificationsBaseEndpointsImpl.class);
	...
	@Context
	protected jakarta.ws.rs.core.HttpHeaders httpHeaders;

	protected String getIfMatchHeader() {
		return httpHeaders != null ? httpHeaders.getHeaderString("If-Match") : null;
	}
	...
	protected Response getInternalServerErrorResponse(Throwable t) {
		...
		MessageListRsrc messageList = new MessageListRsrc();
		return Response.status(Status.INTERNAL_SERVER_ERROR).entity(messageList).build();
	}
