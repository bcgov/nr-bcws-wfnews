...
resource "aws_cloudfront_distribution" "wfnews_geofencing_db" {
  ...
  aliases = ["wfnews-db.${var.target_env}.bcwildfireservices.com"]
  ...
  origin {
    ...
    domain_name = "wfnews-db.${var.license_plate}-${var.target_env}.nimbus.cloud.gov.bc.ca"
    origin_id   = "wfnews_db_${var.target_env}"
  }
  ...
  default_cache_behavior {
    ...
    target_origin_id = "wfnews_db_${var.target_env}"
