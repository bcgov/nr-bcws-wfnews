...
/*
 resource "aws_route53_record" "wfnews_nginx_admin" {
     zone_id = data.aws_route53_zone.zone.id
     name = "wfnews-api-admin.${var.target_env}.bcwildfireservices.com"
     type = "A"
     alias { 
        name = aws_cloudfront_distribution.wfnews_geofencing_nginx_admin[0].domain_name
        zone_id = aws_cloudfront_distribution.wfnews_geofencing_nginx_admin[0].hosted_zone_id
        evaluate_target_health = true
    }
 }

  resource "aws_route53_record" "wfnews_nginx_gui" {
     zone_id = data.aws_route53_zone.zone.id
     name = "wfnews-api-gui.${var.target_env}.bcwildfireservices.com"
     type = "A"
     alias { 
        name = aws_cloudfront_distribution.wfnews_geofencing_nginx_gui[0].domain_name
        zone_id = aws_cloudfront_distribution.wfnews_geofencing_nginx_gui[0].hosted_zone_id
        evaluate_target_health = true
    }
 }
*/
