resource "aws_s3_bucket" "wfnews_upload_bucket" {
  bucket        = "wfnews-${var.license_plate}-${var.target_env}-uploads"
  acl           = "private"
  force_destroy = true
}

resource "aws_s3_bucket" "wfnews_log_bucket" {
  bucket        = "wfnews-${var.license_plate}-${var.target_env}-logs"
  acl           = "private"
  force_destroy = true
}
...
data "aws_s3_bucket" "wfnews_lambda" {
  bucket        = "wfnews-lambda-${var.license_plate}-${var.target_env}"
}

resource "aws_s3_bucket" "wfnews-monitor-queue-bucket" {
  bucket = "wfnews-monitor-queue-${var.license_plate}-${var.target_env}"
